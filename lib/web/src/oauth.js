(function () {
  ...
  function createQueryString(settings) {
    var query = '?callbackUrl=' + encodeURIComponent(settings.callbackUrl) +
  ...
  company.oauth = {
     ...
     * @param settings
     *   - callbackUrl: {string}. Required. The url to the page that receives the authentication data;
    ...
    /**
     * Get the value of the header Authorization that should be passed to the requests that require authentication.
     * If no user is authenticated, null is returned;
     * @returns {string} the Authorization header or null
     */
    getAuthorizationHeader: function () {
      var authData = original.oauth.getAuthenticationData();
      return authData ? 'Bearer ' + btoa(authData.uid + ':' + authData.accessToken) : null;
    },
